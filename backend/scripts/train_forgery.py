...
data_root = os.path.join(os.path.dirname(__file__), '../../data/DocTamper Training')
...
def train_forgery_model(max_scan=50000):
    ...
    if not os.path.exists(images_dir):
        print(f"Warning: Image directory {images_dir} not found. Training on synthetic/simulated ELA features instead.")
        n_samples = 100
        X_clean = np.random.normal(loc=[1.0, 5.0, 2.0], scale=[0.5, 1.0, 0.5], size=(n_samples, 3))
        X_tampered = np.random.normal(loc=[15.0, 80.0, 20.0], scale=[3.0, 10.0, 4.0], size=(n_samples, 3))
        ...
        print(f"Synthetic model saved to {os.path.join(models_dir, 'forgery_model.joblib')}")
        return
